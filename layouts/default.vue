...
onMounted(() => {
  ...
  script1.async = true
  script1.src = 'https://www.googletagmanager.com/gtag/js?id=G-QKQW0MBFH4'
  ...
  <div>
    <header class="z-50 fixed inset-0 h-70px md:h-96px bg-#caefff">
      ...
      <div class="relative h-full md:hidden">
        ...
        <div
          :class="`absolute inset-0 flex items-center justify-between px-4 transition-all duration-300 ease-in-out bg-#caefff z-100 ${
            !openMobileSearch ? '-translate-y-full' : 'translate-y-0'
          }`"
        >
          <div class="relative w-full">
            <input
    ...
    <footer class="pt-0 lg:pt-4">
      <div class="mx-auto px-4 flex flex-col lg:(px-0 w-1024px flex-row)">
        <div class="lg:w-1/2">
          ...
          <p class="flex items-center gap-3"><IconLocal /> Số 01 phố Phạm Văn Bạch, Yên Hòa, Cầu Giấy, Hà Nội.</p>
        </div>
    ...
    <!-- Modal Search -->
    <UModal v-model="modalOptions.open" prevent-close class="!w-30% max-h-90vh">
      <div class="p-30px">
        ...
        <div class="pt-6 pb-4">
          <div class="">
            ...
            <USelect v-model="modalOptions.forSelect" :options="forList" class="mt-2" />
          </div>
          <div class="mt-4">
            <label for="" class="">Thời gian sử dụng</label>
            <USelect v-model="modalOptions.packSelect" :options="packList" class="mt-2" />
          </div>
          <div class="mt-4">
            <label for="" class="">Sắp xếp theo</label>
            <USelect v-model="modalOptions.trendSelect" :options="trendList" class="mt-2" />
          </div>
        </div>
